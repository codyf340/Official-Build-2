  ...
  return (
    <div className="space-y-4">
      {outlooks.map((item, idx) => (
        <div key={idx} className="glass-panel rounded-3xl p-6 hover:bg-white/[0.05] transition-all group">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10 group-hover:scale-110 transition-transform">{getPeriodIcon(item.period)}</div>
              <div><h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 leading-none mb-1">{item.period}</h4><p className="text-[9px] text-slate-600 font-bold uppercase">{item.day}</p></div>
            </div>
            <span className="text-xl font-black text-white">{item.temp}</span>
          </div>
          <p className="text-xs font-black text-white uppercase tracking-wider mb-1">{item.condition}</p>
